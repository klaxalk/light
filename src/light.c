...
bool light_checkOperations()
{
	bool valid = true;
	light_cmd_t op = light_Configuration.operationMode;
...
bool light_parseArguments(int argc, char **argv)
{
	...
	while ((currFlag = getopt(argc, argv, "HhVGSAULIObmclkas:prv:")) != -1) {
		...
		case 'v':
			...
			light_verbosity = (light_loglevel_t)verbosity;
			break;
		}
	}
	...
	if (light_Configuration.operationMode == LIGHT_SET ||
	    light_Configuration.operationMode == LIGHT_ADD || light_Configuration.operationMode == LIGHT_SUB) {
		if (argc - optind != 1) {
		...
		if (light_Configuration.valueMode == LIGHT_PERCENT) {
			...
			light_Configuration.specifiedValuePercent =
			    light_clampPercent(light_Configuration.specifiedValuePercent);
		} else {
...
bool light_initialize(int argc, char **argv)
{
	...
	if (mode == LIGHT_SAVE || (mode == LIGHT_SET && light_Configuration.field == LIGHT_MIN_CAP)) {
		...
		char const *const direrr =
		    "'%s' does not exist and could not be created, make sure this application is run as root.";
	...
	if (light_Configuration.controllerMode == LIGHT_AUTO) {
		...
		LIGHT_ERR_FMT("selected controller '%s' is not valid", light_Configuration.specifiedController);
		return false;
	}
...
bool light_initExecution(unsigned long *rawCurr, unsigned long *rawMax, bool * hasMinCap, unsigned long *minCap)
{
	...
	if (light_Configuration.field == LIGHT_MIN_CAP || light_Configuration.field == LIGHT_MAX_BRIGHTNESS) {
	...
	if (*hasMinCap && *minCap > *rawMax) {
		LIGHT_WARN_FMT("invalid minimum cap (raw) value of '%lu' for controller, ignoring and using 0",
			       *minCap);
...
bool light_execute()
{
	...
	bool hasMinCap;		/* If we have a minimum cap     */
	...
	if (light_Configuration.operationMode == LIGHT_SET ||
		...
		if (light_Configuration.field == LIGHT_MIN_CAP) {
			...
			if (!light_setMinCap
			    (light_Configuration.specifiedController, LIGHT_CLAMP(specValueRaw, 0, rawMax))) {
	...
	fprintf(stderr, "Controller: %s\nValueRaw: %lu\nValuePercent: %.2f\nOpMode: %u\nValMode: %u\nField: %u\n\n",
		...
		light_Configuration.specifiedValuePercent, light_Configuration.operationMode, valueMode,
		light_Configuration.field);
...
bool light_getBrightnessPath(char const *controller, char **path)
{
...
bool light_getMaxBrightnessPath(char const *controller, char **path)
{
...
bool light_controllerAccessible(char const *controller)
{
	...
	if (light_Configuration.controllerMode == LIGHT_AUTO || light_Configuration.field == LIGHT_MAX_BRIGHTNESS) {
	...
	if (light_Configuration.operationMode != LIGHT_GET &&
	    light_Configuration.field != LIGHT_MIN_CAP && !light_isWritable(brightnessPath)) {
...
bool light_prepareControllerIteration(DIR ** dir)
{
	if (!dir) {
...
bool light_iterateControllers(DIR * dir, char *currentController)
{
	struct dirent *file;
...
bool light_listControllers(void)
{
	DIR *dir;
